...
/* Feedbacks */
long int init_fb_Selectrix(long int busnumber, int adres, const char protocol, int index)
{
  if (protocol== 'S')
  {
...
void* thr_sendrec_Selectrix(void *v)
{
  ...
  DBG(busnumber, DBG_INFO, "Selectrix on bus #%ld thread started.", busnumber);
  while (1)
  {
    busses[busnumber].watchdog= 1;
    /* Start/Stop */
