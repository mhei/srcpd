...
void* thr_doInfoClient(void *v) {
	...
	if(infoline[0] == '\0') {
	    for(i=1; i<MAXGAS; i++)
		if(!cmpGA(ga_mm[i], gal[i])) {
		    gal[i]=ga_mm[i];
		    infoGA(ga_mm[i], infoline);
		    break;
		}
	}
	if(infoline[0] == '\0') {
