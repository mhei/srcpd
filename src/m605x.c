...
extern volatile struct _GL ngl[50];  /* neuer, evt geaenderter Wert         */
extern volatile struct _GA ga[MAXGAS];  /* soviele Generic Accessoires gibts             */
extern volatile struct _GA nga[50];  /* soviele Generic Accessoires gibts, neue Werte */
      ...
      for (i = 0; i < 50; i++)
      {
        if (nga[i].id)
        {
          ...
          if (gatmp.action == 1)
          {
            ...
            writeByte (fd, &c, pause_between_bytes);
            SendByte = gatmp.id;
            writeByte (fd, &SendByte, (unsigned long) gatmp.activetime);
            cmd32_pending = 1;
          }
