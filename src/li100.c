...
static int send_command_LI100_SERIAL ( bus_t busnumber, const unsigned char* );
...
#ifdef LI100_USB
static int send_pom_cv_LI100_USB ( bus_t busnumber, int addr, int cv, int value )
#else
static int send_pom_cv_LI100_SERIAL ( bus_t busnumber, int addr, int cv, int value )
#endif
{
  unsigned char byte2send[20];
  unsigned char status;
...
/* program decoder on the main */
#ifdef LI100_USB
static int send_pom_cvbit_LI100_USB ( bus_t busnumber, int addr, int cv, int cvbit,
                               int value )
#else
...
#ifdef LI100_USB
static int send_command_LI100_USB ( bus_t busnumber, const unsigned char *str )
#else
static int send_command_LI100_SERIAL ( bus_t busnumber, const unsigned char *str )
#endif
{
  int i, ctr;
  int status;
...
#endif
{
  ...
  while ( status == -1 )
  {
    ...
    /* 0x62: X-Bus 1 + 2, 0x63: XpressNet */
          ...
          if ( ( buffer[0] == 0x62 ) || ( buffer[0] == 0x63 ) )
          {
    ...
    /*0xa4: Locomotive is being operated by another device
      ...
      if ( ( buffer[0] == 0x83 ) || ( buffer[0] == 0xa3 ) )
      {
        if ( buffer[0] & 0x20 )
      ...
      for ( i = 1; i < ctr; i += 2 )
      {
        ...
        switch ( buffer[i + 1] & 0x60 )
        {
          ...
          case 0x60:    /* illegal decoder parameter; something in error? */
          {
            ...
            for ( i1 = 0;i1 < ctr;i1++ )
            {
              sprintf ( &print_buf[i1*5], "0x%02x ", buffer[i1] );
            }

            print_buf[i1*5] = 0x00;

            syslog ( LOG_INFO, "Illegal feedback received: %s", print_buf );
            break;
          }
        }
      }
