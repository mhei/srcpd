...
static int readAnswer_IB( const long int busnumber, const int generatePrintf );
...
{
  DBG(busnumber, DBG_INFO, "reading configuration for intellibox at bus %ld", busnumber);
    ...
    if ( gatmp.action && ( gatmp.activetime > 0 ) )
    {
      ...
      for ( i1 = 0;i1 < 50;i1++ )
      {
        if ( __ib -> tga[ i1 ].id == 0 )
        {
          ...
          __ib -> tga[ i1 ] = gatmp;
          DBG(busnumber, DBG_DEBUG, "GA %i for switch off at %i,%i on %i", __ib -> tga[ i1 ].id,
