    ...
    char *reply)
{
  ...
  long int lockid;
  int anzparms;
  anzparms = sscanf(parameter, "%ld %ld %ld %ld", &gaddr, &port, &aktion,
       &delay);
  if (anzparms >= 4) {
      /* Port 0,1; Aktion 0,1 */
      /* Only if not locked!! */
  ...
  if (bus_has_devicegroup(bus, DG_SM) && strncasecmp(device, "SM", 2) == 0)
  {
    ...
    if (strcasecmp(ctype, "REG") == 0)
      ...
      if (strcasecmp(ctype, "CV") == 0)
        type = CV;
      else
        if (strcasecmp(ctype, "CVBIT") == 0)
          type = CV_BIT;
        else
          if (strcasecmp(ctype, "PAGE") == 0)
            type = PAGE;
    free(ctype);
    if (type == -1)
      rc = SRCP_WRONGVALUE;
...
int
handleGET(int sessionid, int bus, char *device, char *parameter, char *reply)
{
...
//    sscanf(parameter, "%ld %s %ld %ld", &addr, ctype, &value1, &value2);
  ...
  if (strncasecmp(device, "DESCRIPTION", 11) == 0)
  {
    ...
    else
    {
      ...
      {
        rc = srcp_fmt_msg(SRCP_UNSUPPORTEDDEVICEGROUP, reply, akt_time);
      }
    }
  }
  if (bus_has_devicegroup(bus, DG_LOCK) && (strncasecmp(device, "LOCK", 4) == 0) )
     ...
     char *reply)
{
    ...
    *reply = 0x00;
    ...
    if (bus_has_devicegroup(bus, DG_SESSION) && strncasecmp(device, "SESSION", 7) == 0)
    {
...
int doCmdClient(int Socket, int sessionid)
{
  ...
  nelem = sscanf(line, "%s %ld %s %900c", command, &bus, devicegroup,
       parameter);
  DBG(bus, DBG_INFO, "getting command from session %ld: %s %d %s %s",
      sessionid, command, bus, devicegroup, parameter);
  rc = SRCP_UNKNOWNCOMMAND;
  reply[0] = 0x00;
  if (nelem < 3)
      rc = SRCP_LISTTOOSHORT;
